...
import { Vec2, SPECIES_NAMES } from './types';
...
export interface MissionTarget {
  id: number;
  position: Vec2;
  speciesIndex: number;      // 0-5, which species this target is
  speciesName: string;
  detectionRadius: number;   // how close a robot must be
  status: TargetStatus;
  detectedBy: number[];      // robot IDs that detected it
  classifiedBy: number[];    // robot IDs that correctly classified it
  spawnTick: number;
  expiresAtTick: number;
  drift: Vec2;               // slow movement vector
}
...
interface MissionTypeConfig {
  targetLifetimeTicks: number;
  detectionRadius: number;
  spawnIntervalTicks: number;
  maxActiveTargets: number;
  consensusRequired: number;
  driftMultiplier: number;
  scorePerClassified: number;
  penaltyPerExpired: number;
  initialTargets: number;
}

const TYPE_CONFIGS: Record<MissionType, MissionTypeConfig> = {
  search_classify: {
...
export class MissionManager {
  ...
  private surveyZonesUsed = new Set<number>();
  private rng: () => number;
  ...
  private get config(): MissionTypeConfig {
  ...
  update(currentTick: number, robots: Robot[]): MissionState {
    ...
    if (
      currentTick - this.lastSpawnTick >= cfg.spawnIntervalTicks &&
    ...
    const updatedTargets = this.state.targets.map(target => {
      ...
      if (currentTick >= target.expiresAtTick) {
        return { ...target, status: 'expired' as const };
      }
      ...
      if (this.state.type === 'perimeter') {
        ...
        if (edgeDist > 25) {
          ...
          const toEdge = 0.3;
          if (newPos.x > MAP_W / 2) newPos.x += toEdge;
          else newPos.x -= toEdge;
        }
      }

      let newDetectedBy = [...target.detectedBy];
      let newClassifiedBy = [...target.classifiedBy];
      let newStatus: TargetStatus = target.status;

      for (const robot of robots) {
        if (!robot.isOnline || robot.isJammed) continue;
        ...
        if (dist < target.detectionRadius) {
          ...
          if (
            hdcState.predictedClass === target.speciesIndex &&
    ...
    if (this.state.type === 'survey') {
      const classifiedZones = new Set<number>();
  ...
  private spawnTarget(currentTick: number): void {
    if (this.state.type === 'perimeter') {
      ...
      const candidates = zoneCounts.reduce<number[]>((acc, c, i) => {
  ...
  private spawnStandardTarget(currentTick: number): void {
    const cfg = this.config;
    ...
    const target: MissionTarget = {
      ...
      speciesIndex,
      speciesName: SPECIES_NAMES[speciesIndex],
  ...
  private spawnTargetInZone(currentTick: number, zone: number): void {
    const cfg = this.config;
    const col = zone % 3;
    ...
    const target: MissionTarget = {
      ...
      position: { x: Math.max(5, Math.min(MAP_W - 5, x)), y: Math.max(5, Math.min(MAP_H - 5, y)) },
      speciesIndex: zone,
      speciesName: SPECIES_NAMES[zone],
      detectionRadius: cfg.detectionRadius,
      status: 'active',
      detectedBy: [],
      classifiedBy: [],
  ...
  private spawnPerimeterTarget(currentTick: number): void {
    const cfg = this.config;
    const speciesIndex = Math.floor(this.rng() * 6);
    const edgeMargin = 20;

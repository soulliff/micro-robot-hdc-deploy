...
exports.computeWptFlows = computeWptFlows;
exports.applyWptCharging = applyWptCharging;
 ...
 */
function computeWptFlows(robots) {
    const flows = [];
    ...
    for (const parent of robots) {
        ...
        if (!parent.isOnline)
            continue;
        const params = types_1.SIZE_PARAMS[parent.sizeClass];
        if (params.wptOutputMw <= 0)
            continue;
        const children = parent.childIds
            .map(id => robotMap.get(id))
            .filter((c) => c !== undefined && c.isOnline);
 ...
 */
function applyWptCharging(robots, flows) {
    const robotMap = new Map();
    for (const r of robots) {
        robotMap.set(r.id, r);
    ...
    for (const flow of flows) {
        ...
        const params = types_1.SIZE_PARAMS[child.sizeClass];
